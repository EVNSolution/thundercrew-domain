...
export function RiderForm({ mode = "등록" }: { mode?: "등록" | "수정" }) {
  return (
    <form className="card" aria-label={`라이더 ${mode} 폼`}>
      ...
      <div className="form-actions"><button className="button-secondary" type="button">취소</button><button className="button-primary" type="submit">라이더 {mode}</button></div>
    </form>
  );
}
