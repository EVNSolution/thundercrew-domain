...
export function VehicleForm({ mode = "등록" }: { mode?: "등록" | "수정" }) {
  return (
    <form className="card" aria-label={`차량 ${mode} 폼`}>
      ...
      <div className="form-actions"><button className="button-secondary" type="button">취소</button><button className="button-primary" type="submit">차량 {mode}</button></div>
    </form>
  );
}
