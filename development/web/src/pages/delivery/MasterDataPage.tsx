...
/** 이 모드가 다루는 용도. 배송용 화면이므로 고정이다. */
const PURPOSE: Purpose = 'DELIVERY';
const OTHER_PURPOSE: Purpose = 'CLEANING';
...
const TARGETS: ReadonlyArray<{ id: TargetId; label: string; shared: boolean }> = [
  ...
  { id: 'riders', label: '라이더', shared: false },
...
export function DeliveryMasterDataPage() {
  ...
  const vehicles = useMemo(() => vehiclesByPurpose(fleet.vehicles, PURPOSE), [fleet.vehicles]);
  const riders = useMemo(() => ridersByRole(fleet.riders, 'RIDER'), [fleet.riders]);
  const contracts = useMemo(
    () => contractsForPurpose(fleet.contracts, fleet.vehicles, PURPOSE),
    [fleet.contracts, fleet.vehicles],
  );
  ...
  return (
    <main className="page-content">
      ...
      <p className="sub" style={{ margin: '-6px 0 var(--space-3)' }}>
        "공유" 표시된 대상은 {PURPOSE_LABEL[OTHER_PURPOSE]} 관리와 같은 데이터입니다. 어느 쪽에서
        고쳐도 양쪽에 반영됩니다.
      </p>
          ...
          <section className="page-panel">
            {selectedVehicle ? (
              ...
              <div className="empty-state">
                <b>배송용 차량이 없습니다</b>
                차량을 등록하거나 {PURPOSE_LABEL[OTHER_PURPOSE]} 관리에서 이동해 오세요.
              </div>
            )}
          </section>
        </div>
      ...
      {target === 'contracts' && (
        <section className="page-panel">
          <div className="panel-head">
            <div>
              ...
              <p className="panel-sub">
                배송용 차량 계약 {contracts.filter((c) => !c.terminated).length}건 활성
              </p>
            </div>
          </div>
          ...
          <p className="sub" style={{ marginTop: 10 }}>
            배송용 계약의 인수방식은 <b>인수 / 반납</b>입니다. 클리닝은 직영 / 협력이라, 차량을
            이동하면 이 값이 맞지 않게 됩니다 — 그래서 활성 계약이 이동을 막습니다.
          </p>
        </section>
      )}
    ...
    <section className="page-panel">
      <div className="panel-head">
        <div>
          ...
          <p className="panel-sub">{PURPOSE_LABEL[PURPOSE]} {vehicles.length}대</p>
        </div>
        <input
          className="control"
          style={{ width: 140 }}
  ...
  return (
    <>
      ...
      <MoveField
        ...
        actionLabel={`${PURPOSE_LABEL[OTHER_PURPOSE]}으로 이동`}
        blockers={blockers}
