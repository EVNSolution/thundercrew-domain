...
**결정 2 — 다크모드는 유지합니다.**
DSV에는 다크모드가 없습니다(`prefers-color-scheme` 규칙 0건). 썬더크루에는
`ThemeToggle.tsx`와 다크 토큰 세트가 이미 동작하므로, 제거는 기능 후퇴입니다.
DSV 토큰에 다크 대응값을 추가하는 방향으로 확장합니다. 다크에서도
"색상만으로 상태 전달 금지" 규칙은 동일하게 적용합니다.
...
| 2 | 다크모드 유지 | DSV엔 없습니다. 유지 시 모든 신규 토큰에 다크값이 필요합니다 (§4.1) |
